# Build Docker Container Sandbox Image and Deploy to AWS Lambda API (FastAPI)

param (
    [string]$AWS_ACCOUNT_ID = "",
    [string]$AWS_REGION = "ap-southeast-1",
    [string]$ECR_REPO_NAME = "codexecute-lambda-api",
    [string]$LAMBDA_FUNCTION_NAME = "codeexecute-api"
)
...
Write-Host "Checking ECR Repository ($ECR_REPO_NAME)..." -ForegroundColor Cyan
...
Write-Host "2. Building Docker image for Lambda API (Python, C++, Java, Node.js)..." -ForegroundColor Green
docker buildx build `
...
Write-Host "5. Updating Lambda API ($LAMBDA_FUNCTION_NAME)..." -ForegroundColor Green
...
Write-Host ""
Write-Host "Lambda API Deployment completed successfully!" -ForegroundColor Green
