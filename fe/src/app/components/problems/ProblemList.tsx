	...
	return (
		<div className="max-w-7xl mx-auto px-6 py-8">
			...
			<div className="grid grid-cols-4 gap-6 mb-8">
				<Card className="p-6 bg-gradient-to-br from-green-50 to-green-100"><div className="text-3xl text-gray-900 mb-1">45</div><div className="text-gray-600 text-sm mb-2">Easy Solved</div><div className="w-full bg-green-200 rounded-full h-1.5"><div className="bg-green-500 h-1.5 rounded-full" style={{ width: '45%' }}></div></div></Card>
				<Card className="p-6 bg-gradient-to-br from-amber-50 to-amber-100"><div className="text-3xl text-gray-900 mb-1">32</div><div className="text-gray-600 text-sm mb-2">Medium Solved</div><div className="w-full bg-amber-200 rounded-full h-1.5"><div className="bg-amber-500 h-1.5 rounded-full" style={{ width: '21%' }}></div></div></Card>
				<Card className="p-6 bg-gradient-to-br from-red-50 to-red-100"><div className="text-3xl text-gray-900 mb-1">10</div><div className="text-gray-600 text-sm mb-2">Hard Solved</div><div className="w-full bg-red-200 rounded-full h-1.5"><div className="bg-red-500 h-1.5 rounded-full" style={{ width: '12%' }}></div></div></Card>
				<Card className="p-6 bg-gradient-to-br from-[#1A237E] to-[#283593] text-white"><div className="text-3xl mb-1">87</div><div className="text-white/90 text-sm mb-2">Total Solved</div><div className="w-full bg-white/20 rounded-full h-1.5"><div className="bg-[#00BCD4] h-1.5 rounded-full" style={{ width: '26%' }}></div></div></Card>
			</div>
			...
			<Card className="overflow-hidden">
				<div className="overflow-x-auto">
					<table className="w-full">
						<thead className="bg-gray-50 border-b border-gray-200"><tr><th className="px-6 py-4 text-left text-gray-700 text-sm w-12">Status</th><th className="px-6 py-4 text-left text-gray-700 text-sm">Title</th><th className="px-6 py-4 text-left text-gray-700 text-sm">Difficulty</th><th className="px-6 py-4 text-left text-gray-700 text-sm">Category</th><th className="px-6 py-4 text-left text-gray-700 text-sm">Acceptance</th><th className="px-6 py-4 text-left text-gray-700 text-sm w-32"></th></tr></thead>
						<tbody className="divide-y divide-gray-200">
							{problems.map((problem) => (
								<tr key={problem.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => navigate(`/problems/${problem.id}`)}>
									...
									<td className="px-6 py-4"><span className="text-gray-900 hover:text-[#00BCD4]">{problem.title}</span></td>
									<td className="px-6 py-4"><Badge className={getDifficultyColor(problem.difficulty)}>{problem.difficulty}</Badge></td>
									<td className="px-6 py-4"><div className="flex flex-wrap gap-1">{problem.category.split(', ').map((tag) => (<Badge key={tag} variant="secondary" className="bg-gray-100 text-gray-700 text-xs">{tag}</Badge>))}</div></td>
									<td className="px-6 py-4 text-gray-700">{problem.acceptance}</td>
									<td className="px-6 py-4"><Button size="sm" className="bg-[#00BCD4] hover:bg-[#00ACC1] text-white" onClick={(e) => { e.stopPropagation(); navigate(`/problems/${problem.id}`); }}>Solve</Button></td>
								</tr>
							))}
						</tbody>
					</table>
			...
			<Card className="mt-6 overflow-hidden">
				<div className="bg-gradient-to-r from-[#1A237E] to-[#283593] p-6 text-white"><div className="flex items-center justify-between"><div className="flex items-center gap-4"><div className="w-16 h-16 bg-white/20 rounded-lg flex items-center justify-center"><TrendingUp className="w-8 h-8" /></div><div><h3 className="mb-1">Daily Challenge</h3><p className="text-white/90 text-sm">Solve today's problem and earn bonus points!</p></div></div><Button className="bg-[#00BCD4] hover:bg-[#00ACC1] text-white">Start Challenge</Button></div></div>
			</Card>
		</div>
	);
}